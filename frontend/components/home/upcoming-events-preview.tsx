...
export function UpcomingEventsPreview() {
  const events = useEventStore((state) => state.events).filter(e => e.isActive).slice(0, 4)
    ...
    <section className="py-16 lg:py-24 bg-muted/30">
      <div className="mx-auto max-w-7xl px-4 lg:px-8">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {events.length === 0 && (
            <Card className="sm:col-span-2 lg:col-span-4 border-dashed border-border/60">
              <CardContent className="p-8 text-center text-sm text-muted-foreground">
                Upcoming events will show up here once mosques publish their schedules.
              </CardContent>
            </Card>
          )}
