...
export function NearbyMosquesPreview() {
  const mosques = useMosqueStore((state) => state.mosques).slice(0, 3)
      ...
      <CardContent className="space-y-4">
        {mosques.length === 0 && (
          <div className="rounded-lg border border-dashed border-border/60 bg-muted/20 p-6 text-center text-sm text-muted-foreground">
            Mosque listings will appear here as soon as the directory data is available.
          </div>
        )}
        {mosques.map((mosque) => (
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1 text-sm text-muted-foreground mt-1">
                      ...
                      <span className="truncate">
                        {[mosque.city, mosque.state].filter(Boolean).join(', ') || mosque.address}
                      </span>
                    </div>
                  </div>
