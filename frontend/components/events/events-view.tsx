...
import type { EventCategory, Event } from '@/lib/types'
...
export function EventsView() {
  ...
  }, [loadEvents, loadMosques])

  const filteredEvents = useMemo(() => {
    let result = [...eventsFromStore].filter(e => e.isActive)
    ...
    return result
  }, [eventsFromStore, searchQuery, selectedCategory, sortBy])
      ...
      {eventsError ? (
        <Card>
          <CardContent className="py-12 text-center text-sm text-muted-foreground">
            We couldn&apos;t load the event schedule right now.
          </CardContent>
        </Card>
          ...
          {Object.entries(eventsByDate).map(([date, events]) => (
            <div key={date}>
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {events.map((event) => (
                  <EventCard key={event.id} event={event} mosques={mosques} />
                ))}
              </div>
            </div>
          ))}
        ...
        <div className="space-y-4">
          {filteredEvents.map((event) => (
            <EventListItem key={event.id} event={event} mosques={mosques} />
          ))}
        </div>
      )}
    </div>
...
function EventCard({ event, mosques }: { event: Event, mosques: any[] }) {
...
function EventListItem({ event, mosques }: { event: Event, mosques: any[] }) {
  const mosque = mosques.find(m => m.id === event.mosqueId)
