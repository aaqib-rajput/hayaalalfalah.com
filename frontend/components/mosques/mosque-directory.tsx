...
export async function getMosques(): Promise<Mosque[]> {
  return useMosqueStore.getState().loadMosques()
}
...
export function MosqueDirectory() {
  ...
  const loading = useMosqueStore((state) => state.loading)
  const error = useMosqueStore((state) => state.error)
  ...
  const filteredMosques = useMemo(() => {
    ...
    return result
  }, [mosques, searchQuery, sortBy, selectedFacilities, verifiedOnly])
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        const nearby = mosques
          .filter((mosque) => mosque.latitude !== 0 || mosque.longitude !== 0)
          .map(mosque => ({
            ...mosque,
            distance: calculateDistance(coords.lat, coords.lng, mosque.latitude, mosque.longitude)
          }))
  ...
  useEffect(() => {
    if (location) {
      const nearby = mosques
        ...
        .map(mosque => ({
          ...mosque,
          distance: calculateDistance(location.lat, location.lng, mosque.latitude, mosque.longitude)
        }))
        .filter(mosque => mosque.distance <= radius)
        .sort((a, b) => a.distance - b.distance)
      setNearbyMosques(nearby)
        ...
        <TabsContent value="all" className="mt-6 space-y-6">
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20">
              ...
              <p className="mt-4 text-muted-foreground">Loading mosques...</p>
            </div>
          ) : error ? (
                          ...
                          <Link href={`/mosques/${mosque.id}`} className="flex-1 p-4">
                            <div className="flex items-start justify-between gap-4">
                              <div className="flex-1 min-w-0">
                                ...
                                {mosque.facilities.slice(0, 4).map((facility: string) => (
