...
import type { EventCategory, Event, Mosque } from '@/lib/types'
...
export async function generateMetadata({ params }: EventPageProps) {
  ...
  try {
    const event = await fetchApi<Event>(`/events/${id}`)
    return {
      title: `${event.title} | MosqueConnect`,
      description: event.description,
    }
  } catch (error) {
    return { title: 'Event | MosqueConnect' }
  }
}
...
export default async function EventPage({ params }: EventPageProps) {
  ...
  try {
    const event = await fetchApi<Event>(`/events/${id}`)
    const mosque = await fetchApi<Mosque>(`/mosques/${event.mosqueId}`)
