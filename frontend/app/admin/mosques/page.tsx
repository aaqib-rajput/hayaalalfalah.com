...
export default function AdminMosquesPage() {
  const { mosques, addMosque, updateMosque, deleteMosque } = useMosqueStore()
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      toast.error("Failed to remove mosque")
    }
  }

  const handleSave = async (formData: FormData) => {
    const mosqueData: Omit<Mosque, 'id' | 'createdAt' | 'updatedAt'> = {
    ...
    if (editingMosque) {
      try {
        await updateMosque(editingMosque.id, mosqueData)
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            Registered Mosques ({mosques.length})
          </CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
