...
export default function AdminDashboardPage() {
  ...
  const totalDonations = financeRecords
  ...
  const totalMembers = mosques.reduce((sum, m) => sum + (m.memberCount || 0), 0)
  ...
  const recentTransactions = financeRecords
          ...
          <CardContent>
            <div className="space-y-4">
              {recentTransactions.map((record) => {
                const mosque = mosques.find(m => m.id === record.mosqueId)
                return (
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent className="space-y-6">
            {mosques.slice(0, 3).map((mosque) => {
              const donations = financeRecords
