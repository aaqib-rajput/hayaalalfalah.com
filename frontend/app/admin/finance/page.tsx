...
export default function AdminFinancePage() {
  const { records, goals, addRecord, addGoal } = useFinanceStore()
  ...
  const donationList = records.filter(r => r.type === 'donation')
  ...
  const handleAddDonation = async (formData: FormData) => {
    const donationData: Omit<FinanceRecord, 'id' | 'createdAt'> = {
  ...
  const handleAddExpense = async (formData: FormData) => {
    const expenseData: Omit<FinanceRecord, 'id' | 'createdAt'> = {
      mosqueId: formData.get("mosqueId") as string,
      type: "expense",
      amount: parseFloat(formData.get("amount") as string),
      category: formData.get("category") as FinanceRecord["category"],
  ...
  const handleAddGoal = async (formData: FormData) => {
    const goalData: Omit<DonationGoal, 'id'> = {
      mosqueId: formData.get("mosqueId") as string,
