...
export default function AdminImamsPage() {
  const { imams, mosques, addImam, updateImam, deleteImam } = useMosqueStore()
  ...
  const handleSave = async (formData: FormData) => {
    ...
    const imamData: Omit<Imam, 'id' | 'createdAt'> = {
    ...
    if (editingImam) {
      try {
        await updateImam(editingImam.id, imamData)
        toast.success("Imam profile updated successfully")
      } catch {
        toast.error("Failed to update imam profile")
        return
      }
    } else {
      try {
        await addImam(imamData)
        toast.success("New imam profile added successfully")
      } catch {
        toast.error("Failed to add imam profile")
        return
      }
    }
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      toast.error("Failed to remove imam profile")
    }
  }

  const activeImams = imams.filter(i => i.isActive).length
  const totalExperience = imams.reduce((sum, i) => sum + i.yearsOfExperience, 0)
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{imams.length}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{new Set(imams.map(i => i.mosqueId)).size}</div>
          </CardContent>
        </Card>
      </div>
